...
import { Moon, Star, Brain, Ticket, Medal, CalendarClock, CheckCircle2, Crown, Users, Clock, Zap, Bell, WifiOff, RefreshCw, Calendar, Bed, Sun, ChevronDown, ChevronUp, Activity, Upload, Sparkles, AlertCircle, CheckCircle, Edit3, Loader2, Camera, TrendingUp, TrendingDown, BarChart3 } from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ user }) => {
  ...
  const fetchData = async () => {
    ...
    // Fetch recent logs for sleep insights charts (last 14 days)
    try {
      const allUserLogs = await db.getUserLogs(user.id);
      const last14 = allUserLogs
        .filter((l: any) => !l.bonus_type)
        .slice(0, 14)
        .reverse(); // oldest first for charts
      setRecentLogs(last14);
    } catch (err) {
      console.error('Failed to fetch recent logs for insights:', err);
    }
  };
      ...
      <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
        ...
        >
          <h3 className="text-lg font-bold text-gray-900 flex items-center">
            <BarChart3 className="mr-2 text-indigo-500" /> My Sleep Insights
          </h3>
          {showInsights ? <ChevronUp size={20} className="text-gray-400" /> : <ChevronDown size={20} className="text-gray-400" />}
        </button>

        {showInsights && recentLogs.length >= 2 && (
          <div className="mt-4 space-y-6">
            ...
            {/* --- AI Analysis Card --- */}
            <div className="bg-gradient-to-br from-violet-50 to-indigo-50 rounded-xl border border-violet-200 p-4">
              <div className="flex items-center justify-between mb-3">
        ...
        {showInsights && recentLogs.length < 2 && (
          <div className="mt-4 text-center py-6 text-gray-400">
            ...
            <p className="text-sm">Log at least 2 nights to see your sleep trends!</p>
          </div>
        )}
      </div>
